    ...
    def detect_seasonal_anomalies(self, 
        ...
        if timestamp is not None:
            ...
            for i, ts in enumerate(timestamp):
                # Extract appropriate seasonality component from timestamp
                # Example: hour of day for daily seasonality
        ...
        for pos in range(period):
            ...
            if np.sum(pos_indices) > 5:  # Need enough samples
                for feat in range(n_features):
                    ...
                    seasonal_anomalies[pos_indices] |= np.abs(data[pos_indices, feat] - pos_mean) > 3 * pos_std
